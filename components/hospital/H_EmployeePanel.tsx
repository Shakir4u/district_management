...
import { Employee } from '../../hospital-types';
import Card from '../ui/Card';
import Input from '../ui/Input';
import Button from '../ui/Button';
import Select from '../ui/Select';

interface H_EmployeePanelProps {
    employees: Employee[];
...
const H_EmployeePanel: React.FC<H_EmployeePanelProps> = ({ employees, onUpdate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isEditing) {
      onUpdate(employees.map(p => p.id === isEditing ? { ...formData, id: isEditing } as Employee : p));
    } else {
      onUpdate([...employees, { ...formData, id: Date.now().toString() } as Employee]);
    }
    handleCancel();
  };
...
export default H_EmployeePanel;